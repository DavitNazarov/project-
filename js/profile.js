...
function removePost(postId) {
  ...
  if (postToRemove) {
    ...
    // // localStorage.removeItem("userPostsInput");
    // // localStorage.removeItem("userTextarea");
  }
}
...
// This is JavaScript code that adds functionality to the web page
// Get the elements from the HTML document by their id
let myForm = document.getElementById("myForm");
let myInput = document.getElementById("myInput");
let myButton = document.getElementById("myButton");
let mytextarea = document.getElementById("floatingTextarea");
let myInputFile = document.getElementById("user_choosen_image");
// Define a function that sends a post request and adds a div element
function addPostRequest(event) {
  // Prevent the default behavior of the form submission
  event.preventDefault();
  // Get the value of the input
  let myInputValue = myInput.value;
  let mytextareaValue = mytextarea.value;
  // Check if the input is not empty
  if ((myInputValue, mytextareaValue)) {
    let postContent0 = document.querySelector(".post_grid");
    ...
    <p>${mytextareaValue}</p>
      ...
      `;
    postContent0.appendChild(post);
    localStorage.setItem("userPostsInput", myInputValue);
    localStorage.setItem("userTextarea", mytextareaValue);
    ...
    // Display a message that the input is empty
    alert("Please enter some data");
  }
}
// Add a submit event listener to the form that calls the addPostRequest function
myForm.addEventListener("submit", addPostRequest);
...
<div class="post_box" id="post_box">
  ...
  <div class="post_content">    
    <div class="edit" onclick="openFileInput(this)">
      ...
      <div>
        <svg id="delete_icon choosen_delete_icon" fill="#000000" version="1.1" id="Layer_1"
            xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
            viewBox="0 0 512 512" xml:space="preserve" width="64px" height="64px">
            <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
            <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
...
<h1>${localStorage.getItem("userPostsInput")}</h1>
<p>${localStorage.getItem("userTextarea")}</p>

  `)
    : null;
}

postConteiner1.appendChild(getPost);
let user_input_post_add_file = document.getElementById(
  "user_input_post_add_file"
);
user_input_post_add_file.addEventListener("change", function () {
  const read = new FileReader();

  read.addEventListener("load", () => {
    localStorage.setItem("user_choosen_image", read.result);
  });
